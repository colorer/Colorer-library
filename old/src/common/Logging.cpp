...
static void file_logger(int level, const char *cname, const char *msg, va_list v){
  ...
  while (logg == 0 && idx < 10){
    char log_name[30];
#ifdef __unix__
    sprintf(log_name, "/tmp/clr-trace%d.logg", idx);
#else
    sprintf(log_name, "c:\\clr-trace%d.logg", idx);
#endif
    logg = fopen(log_name, "ab+");
    idx++;
  }

  fprintf(logg, "[%s][%s] ", levelNames[level], cname);
