...
FileInputSource::FileInputSource(const String *basePath, FileInputSource *base){
  bool prefix = true;
  if (basePath->startsWith(ÑString("file://"))){
    baseLocation = new SString(basePath, 7, -1);
  }else if (basePath->startsWith(ÑString("file:/"))){
  ...
  delete[] temp;
#endif
  if(prefix && (baseLocation->indexOf(':') == -1 || baseLocation->indexOf(':') > 10) && !baseLocation->startsWith(ÑString("/"))){
    SString *n_baseLocation = new SString();
    n_baseLocation->append(ÑString("/")).append(baseLocation);
    delete baseLocation;
