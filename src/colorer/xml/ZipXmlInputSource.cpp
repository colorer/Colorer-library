...
{
  if (!path || *path == '\0') {
    throw InputSourceException(ÑString("Can't create jar source"));
  }
  if (xercesc::XMLString::startsWith(path, kJar)) {
    int path_idx = xercesc::XMLString::lastIndexOf(path, '!');
    if (path_idx == -1) {
      throw InputSourceException(SString("Bad jar uri format: ") + ÑString(path));
    }
    ...
    in_jar_location.reset(new SString(ÑString(path), path_idx + 1, -1));
    ...
    UString in_base(new SString(ÑString(base), base_idx + 1, -1));
    ÑString d_path = ÑString(path);
  ...
  SString str("jar:");
  str.append(ÑString(jar_input_source->getInputSource()->getSystemId()));
  str.append(ÑString("!"));
