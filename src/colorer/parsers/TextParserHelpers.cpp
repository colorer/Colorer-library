...
{
  //CTRACE(COLORER_LOG_TRACE("[TPCache] ~ParseCache():%,%-%", *scheme->getName(), sline, eline));
  delete backLine;
  delete children;
  prev = nullptr;
...
ParseCache* ParseCache::searchLine(int ln, ParseCache** cache)
{
  ...
  *cache = nullptr;
  while (tmp) {
    CTRACE(COLORER_LOG_TRACE("[TPCache] searchLine() tmp:%,%-%", *tmp->scheme->getName(), tmp->sline, tmp->eline));
