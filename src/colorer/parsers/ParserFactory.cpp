...
SString ParserFactory::searchCatalog() const
{
  ...
  if (right_path.length() == 0) {
    ...
    throw ParserFactoryException(ÑString("Can't find suitable catalog.xml file. Check your program settings."));
  }
  return right_path;
}
...
void ParserFactory::getPossibleCatalogPaths(std::vector<SString> &paths) const
{
  ...
  if (hmod) {
    ...
    if (len > 0) {
      ÑString module(cname, 0, len - 1);
      int pos[2];
      pos[0] = module.lastIndexOf('\\');
      pos[1] = module.lastIndexOf('\\', pos[0]);
      for (int idx = 0; idx < 2; idx++)
        if (pos[idx] >= 0) {
          paths.emplace_back(SString(ÑString(module, 0, pos[idx])).append(ÑString("\\catalog.xml")));
        }
    }
  }
  ...
  if (home_drive && home_path) {
    try {
      SString d = SString(home_drive).append(ÑString(home_path)).append(ÑString("/.colorer5catalog"));
...
void ParserFactory::getPossibleCatalogPaths(std::vector<SString> &paths) const
{
  ...
  if (home_path != nullptr) {
    try {
      TextLinesStore tls;
      tls.loadFile(&StringBuffer(home_path).append(ÑString("/.colorer5catalog")), nullptr, false);
      if (tls.getLineCount() > 0) {
        paths.emplace_back(SString(tls.getLine(0)));
      }
    } catch (InputSourceException &) { //-V565
  ...
  paths.emplace_back(SString(ÑString("/usr/share/colorer/catalog.xml")));
  paths.emplace_back(SString(ÑString("/usr/local/share/colorer/catalog.xml")));
}
#endif
...
void ParserFactory::loadCatalog(const String* catalog_path)
{
  if (!catalog_path) {
    ...
    if (base_catalog_path.length() == 0) {
      throw ParserFactoryException(ÑString("Can't find suitable catalog.xml file."));
    }
  } else {
...
StyledHRDMapper* ParserFactory::createStyledMapper(const String* classID, const String* nameID)
{
  ...
  const ÑString class_default("rgb");
  if (classID == nullptr) {
  ...
  const ÑString name_default("default");
  ÑString name_env;
  if (nameID == nullptr) {
    char* hrd = getenv("COLORER5HRD");
    if (hrd != nullptr) {
      name_env = ÑString(hrd);
      name_id = &name_env;
    } else {
  ...
  for (size_t idx = 0; idx < hrd_node->hrd_location.size(); idx++)
    if (hrd_node->hrd_location.at(idx).length() != 0) {
      ...
      try {
        ...
        LOG(ERROR) << e.what();
        throw ParserFactoryException(ÑString("Error load hrd"));
      }
    }
  return mapper;
}
