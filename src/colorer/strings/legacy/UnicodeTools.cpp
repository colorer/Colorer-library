...
bool UnicodeTools::getNumber(const UnicodeString* pstr, double* res)
{
  double Numr, r, flt;
  int pos, Type, Num;
...
bool UnicodeTools::getNumber(const UnicodeString* pstr, int* res)
{
  double dres;
  if (getNumber(pstr, &dres))
    *res = (int) dres;
  else
    return false;
  return true;
